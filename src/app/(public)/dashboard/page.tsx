...
// import { useTrainingStore } from "@/features/dashboard/dashboard.store";

// export default function DashboardPage() {
//   const router = useRouter();
//   const { modules, fetchModules, loading } = useTrainingStore();
...
//               Providing transportation for children with special needs requires patience, awareness, and responsibility.
//             </p>
//           </div>
//           <div className="relative w-full md:w-[350px] lg:w-[400px] h-48 md:h-auto overflow-hidden">
//              <img src="/images/child.png" alt="banner" className="absolute inset-0 w-full h-full object-cover object-center" />
//           </div>
//         </div>
...
//                   onClick={() => router.push(mod.href)} // Correct Next.js Navigation
//                 />
//               ))
//             )}
//           </div>
...
// export default function DashboardPage() {
//   const router = useRouter();
//   const { dashboardData, fetchDashboardMetrics, loading, error } = useDriverDashboardStore();
...
//             {error && (
//               <p className="text-sm font-semibold text-red-500 bg-red-50 p-3 rounded-xl">{error}</p>
//             )}

//             {loading ? (
//               <p className="text-sm font-bold text-slate-400 italic animate-pulse">Loading...</p>
//             ) : modulesList.length === 0 ? (
//               <p className="text-sm text-slate-400 italic">No operational modules found.</p>
//             ) : (
//               modulesList.map((mod) => (
//                 <ModuleDetailedCard
//                   key={mod.id}
...
//                   description={mod.description}
//                   buttonText={mod.moduleProgressPercent === 100 ? "Completed" : "Start Learning"}
//                   buttonVariant={mod.moduleProgressPercent === 100 ? "secondary" : "primary"}
//                   src={mod.thumbnailUrl ? `${process.env.NEXT_PUBLIC_S3_BUCKET_URL}/${mod.thumbnailUrl}` : "/images/login-hero.png"}
//                   onClick={() => router.push(`/modules/${mod.id}`)} // Updated routing to match unique backend IDs
//                 />
//               ))
//             )}
//           </div>
